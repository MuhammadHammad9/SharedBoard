...
| **D-6** | The marquee rectangle is assigned to two different layers. FLOWS §8.2.3 says "drag on empty canvas → marquee rectangle on the **overlay** layer"; the §14.3 layer-stack table lists "the marquee rectangle" under **layer 2 (interaction)**.                                                                                                                                                                                                                                      | `docs/02-FLOWS.md` §8.2.3 and §14.3             | **§14.3 wins.** It is the authoritative specification of the layer stack, it is the more specific of the two statements, and the implementation plan already resolved it that way. The marquee renders on **layer 2**; selection boxes and handles render on **layer 3**, as §14.3 also states. The practical difference matters: layer 2 is cleared and redrawn on every pointermove of an interaction, which is exactly the marquee's update rate, while layer 3 additionally carries remote cursors from Phase 10. Applied in Phase 4.                                                            |
| **D-7** | TRD §8.4's grouping table has **no row for the keyboard nudge**. `FR-CANVAS-011` gives arrow keys a 1-unit nudge and Shift+arrow 10, and OS key repeat fires the handler roughly thirty times a second — so a two-second press produces sixty history entries and needs sixty Ctrl+Zs to undo. The table predates the requirement; it is a gap rather than a contradiction.                                                                                                       | `docs/03-TRD.md` §8.4                           | **Treated as the typing row.** A held arrow key coalesces into one entry per burst, keyed on the selection and using the same 1 s window §8.4 specifies for a sticky note. The two cases have identical shape — a repeating key producing many small UPDATEs on a fixed target — and applying the existing rule is a smaller invention than leaving the nudge ungrouped, which PRD risk `R-3`'s "do not invent a variant" instruction is aimed at. Keying on the selection means nudging a different object starts a fresh entry. Applied in Phase 6; see `features/canvas/history/grouping.ts`.     |
| **D-8** | Two specified behaviours compete for the same focus after a failed login. FLOWS §3.2 says form-level errors "receive focus for screen readers"; FLOWS §4 branch 3b says a wrong password must clear the password field and "focus returns to password". Both cannot hold — whichever runs last wins, and in practice the error banner mounts after the field is refocused and silently steals it.                                                                                 | `docs/02-FLOWS.md` §3.2 and §4                  | **§4 branch 3b wins, for the credential-failure case only.** It is the more specific rule, and nothing is actually lost: the banner keeps `role="alert"`, so a screen reader still announces it — only the caret position differs. Putting the caret in the password field is also the better outcome for the same user, who otherwise has to navigate back to it before they can retype. `FormError` gained an `autoFocus` prop defaulting to true; Login passes false on this one branch. Every other form-level error in the product still takes focus as §3.2 requires. Applied in Phase 7.      |
| **D-9** | TRD §4.2 specifies `DELETE /boards/:id/permanent` with a required `{confirmName}` body. A DELETE carrying a required body is unreliable end to end — several proxies, CDNs and `fetch` implementations drop it — and a confirmation that can be silently discarded in transit is not a confirmation: the request would arrive as an unconfirmed permanent delete. | `docs/03-TRD.md` §4.2 | **Implemented as `POST /boards/:id/permanent-delete`.** Same semantics, same exact-name check, a method whose body is universally carried. Every other path in the §4.2 table is implemented verbatim. Applied in Phase 8. |
| **D-10** | `R-SYNC-013` forbids `SELECT MAX(seq)` outside a transaction, but the same read-then-write race applies to `Board.objectCount`, which no rule mentions. Two concurrent appends both read `objectCount = 40`, both write `41`, and the board's count silently drifts below the truth — 40 concurrent batches lost 11 objects from the count in the Phase 8 concurrency test. | `docs/03-TRD.md` §5.4 | **`objectCount` is adjusted in SQL inside the same statement that claims the sequence block** (`SET "objectCount" = GREATEST(0, "objectCount" + $delta)`), and the `MAX_OBJECTS_PER_BOARD` cap is checked against the value the database returned, so exceeding it rolls the transaction back. Treat every denormalised counter this way, not only `seq`. Applied in Phase 8. |
| **D-11** | Nothing in the specification says what op id an undo carries. A history entry holds ONE `inverse` array and ONE `forward` array, and undo → redo → undo replays the identical objects. Because the op id is the server's idempotency key (`R-SYNC-014`), the second undo is re-acked as a duplicate and writes nothing — visible locally, absent from the database, and only detectable after a reload. | `docs/03-TRD.md` §8.3 | **Every re-emit is given a fresh op id at the send boundary.** The `objectId` and payload are untouched, so the change the server applies is exactly the change the user made; only the wire identity is new. Applied in Phase 8. |

| **D-12** | Nothing in the specs says how the bundle gate should treat an ASYNC chunk. The gate summed every non-board chunk into "initial", which was accurate while everything except the board loaded eagerly. The moment Framer Motion moved behind a dynamic import it counted 38 KB gzipped against a budget those bytes do not spend, over-reporting the initial bundle by ~40 KB. | `docs/01-PRD.md` §7.1 | **The initial set is read from the built `index.html`** — the entry `<script type="module">` plus its `modulepreload` links, which is exactly what the browser fetches before first render. Async chunks are printed but not budgeted. A gate that over-reports is not "safely conservative"; it is a number nobody can act on, and it eventually fails a build for bytes no user downloads. Applied in Phase 8b. |

| **D-13** | TRD §6.2's `applyOp` keeps tombstones as a bare `Set` and drops every CREATE for a tombstoned id. That is right about CONCURRENT ops and wrong about later ones: delete X, then press Ctrl+Z. Undo emits an ordinary CREATE (`R-UNDO-003`) which the server stores at a higher seq and broadcasts — and every other client drops it. The object returns for the person who undid and for nobody else, a permanent divergence from the most ordinary action there is. | `docs/03-TRD.md` §6.2 | **Tombstones record the seq of the delete.** An op at or below it lost the race and is dropped; an op above it is a later decision, applies, and clears the tombstone. A local op has no seq yet — it is the user's own current intent — and always applies. Capped at `TOMBSTONE_CAP` (`R-PERF-023`). Applied in Phase 9b. |
| **D-14** | `clearLoginAttempts` cleared only the per-EMAIL counter. `assertLoginAllowed` increments the per-email AND per-IP counters on every attempt, before the password is checked, so the IP counter accumulated on **successful** logins too: an office behind one NAT reached 20 logins in fifteen minutes and the twenty-first person got a 429 with a correct password. The rule the code documents — "counted on FAILURE only, and cleared on success" — was not what it did. | `docs/03-TRD.md` §11.2 | **Success clears both counters.** The spray defence is untouched: an attacker trying many accounts is failing, and failures still accumulate. Two regression tests: 25 consecutive successful logins from one address all return 200, and 25 failed ones still trip the limiter. Found when Phase 9b's e2e suite, which signs in twice per test from one address, started failing on `login 429`. Applied in Phase 9b. |

| **D-15** | Framer Motion's `layout` prop on the dashboard grid — added in Phase 8b, outside the Phase 8 motion table — makes surviving cards slide to new positions for 300 ms after any list change. A `⋮` menu clicked during that window misses, because the button moves out from under the pointer between the hit test and the dispatch. The Phase 10 e2e suite caught it under parallel load; a user on a trackpad is in the same position. | Phase 8 motion table, anti-pattern `A-70` | **`layout` removed.** Entry, exit, hover and the stagger stay. It is the same complaint `A-70` already bans hover scale for — "a card that grows under the cursor shifts its neighbours and the grid twitches" — with a longer duration. Applied in Phase 10. |
