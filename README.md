...
> **Status: Phase 8a complete — boards persist.** M1 shipped the whole canvas: pan and zoom, the pen, selection and transforms, every object type bar images, and per-user undo/redo. Phase 7 added REST authentication with refresh rotation and reuse detection, route guards, and the five auth screens. Phase 8a adds the persistence half of M2 — the `Board` / `BoardMember` / `Operation` / `Snapshot` models, the op log with transactional gap-free sequence assignment, snapshots off the request path, the twelve board endpoints, and a client outbox that survives a reload and an offline spell. Draw, refresh, and the work is there (`AT-10`). The dashboard is Phase 8b. See [`docs/04-IMPLEMENTATION-PLAN.md`](./docs/04-IMPLEMENTATION-PLAN.md).
>
