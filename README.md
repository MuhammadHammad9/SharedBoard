...
> **Status: Phase 8 complete — milestone M2 "It persists" is done.** M1 shipped the whole canvas: pan and zoom, the pen, selection and transforms, every object type bar images, and per-user undo/redo. Phase 7 added REST authentication with refresh rotation and reuse detection, route guards, and the five auth screens. Phase 8 makes boards real end to end: the op log with transactional gap-free sequence assignment, snapshots off the request path, twelve board endpoints, a client outbox that survives a reload and an offline spell, and the dashboard — all seven states, sort, filter, debounced search, Trash with restore and typed-name permanent delete, duplicate, and inline rename. Draw, refresh, and the work is there (`AT-10`). Real-time sync is Phase 9. See [`docs/04-IMPLEMENTATION-PLAN.md`](./docs/04-IMPLEMENTATION-PLAN.md).
>
