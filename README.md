...
> **Status: Phase 9a complete — the WebSocket gateway is live.** M1 shipped the whole canvas and M2 made boards persist: auth, the op log, snapshots, the outbox and the dashboard. Phase 9a adds the server half of real-time — ticket-authenticated upgrades, rooms with round-robin presence colours, the seven-step `handleOp` from TRD §5.4, 16 ms broadcast batching, heartbeat termination and Redis pub/sub fan-out. Proven by a socket suite driving a real `ws` client against a real Postgres, including 100 concurrent writes over five sockets producing a gap-free sequence. The client `SyncEngine` is 9b. See [`docs/04-IMPLEMENTATION-PLAN.md`](./docs/04-IMPLEMENTATION-PLAN.md).
>
